...
#
# from Depthformer.ops.wrappers import resize
#
from Models.BaseNN.Models.Depthformer.builder import NECKS
...
class HAHIHeteroNeck(BaseModule):
    ...
    def forward(self, inputs):
        ...
        # HI (deformable self attention) 这个针对的是从Swintransformer出来的层级结构
        masks = []
        src_flattens = []
        mask_flatten = []
        spatial_shapes = []
        ...
        for i in range(len(feats_trans)):
            ...
            pos = self.trans_positional_encoding(mask=mask)
            mask = mask.flatten(1)
        ...
        # HA (deformable cross attention) 这个针对的是从RestNet出来的单张特征
