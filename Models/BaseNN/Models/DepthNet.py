...
class DepthDecoder(nn.Module):
    ...
    def forward(self, input_features):
        ...
        # decoder
        x = input_features[-1]  # 只取最后一维

        for cnt, i in enumerate(range(4, -1, -1)):  # cnt 与 i 的取值分别是0,4；1,3；2,2,3,1；4,0。所以upconvs0是正序
            x = self.upconvs0[cnt](x)
            x = [upsample(x)]
            if self.use_skips and i > 0:  # 除了最后一个不进行cat
                x += [input_features[i - 1]]
            x = torch.cat(x, 1)  # 将一个list 数组变成tensor
            x = self.upconvs1[cnt](x)
            if i in self.scales:  # range(4) 0,1,2,3 除了第一次循环不进行disconvs的选择输出
...
if __name__ == "__main__":
    ...
    tgt_img = torch.randn(B, 3, 320, 320).cuda()
